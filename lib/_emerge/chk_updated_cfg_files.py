...
def chk_updated_cfg_files(eroot, config_protect):
    ...
    for x in result:
        writemsg_level(f"\n {colorize('WARN', '* ' + _('IMPORTANT:'))} ", level=logging.INFO, noiselevel=-1, )
        if not x[1]:  # it's a protected file
            writemsg_level(f"config file '{x[0]}' needs updating.\n", level=logging.INFO, noiselevel=-1, )
        else:  # it's a protected dir
            if len(x[1]) == 1:
                head, tail = os.path.split(x[1][0])
                tail = tail[len("._cfg0000_"):]
                fpath = os.path.join(head, tail)
                writemsg_level(f"config file '{fpath}' needs updating.\n", level=logging.INFO, noiselevel=-1, )
            else:
                writemsg_level(f"{len(x[1])} config files in '{x[0]}' need updating.\n",
                               level=logging.INFO,
                               noiselevel=-1,
                               )
