...
def pkg_use_display(pkg, opts, modified_use=None):
    ...
    forced_flags = set(chain(pkg.use.force, pkg.use.mask))
    ...
    for varname in use_expand:
        ...
        for f in use:
            if f.startswith(flag_prefix):
                ...
                use_enabled.setdefault(varname.upper(),
                                       []).append(_flag_info(f, f[len(flag_prefix):]))

        for f in pkg.iuse.all:
            if f.startswith(flag_prefix):
                use_expand_flags.add(f)
                if f not in use:
                    use_disabled.setdefault(varname.upper(),
                                            []).append(_flag_info(f, f[len(flag_prefix):]))
    ...
    for f in pkg.iuse.all:
        if f not in use and f not in use_expand_flags:
