...
class fakedbapi(dbapi):
    ...
    def _instance_key_multi_instance(self, cpv, support_string=False):
        try:
            return (cpv, cpv.build_id, cpv.file_size, cpv.build_time, cpv.mtime)
        ...
        for pkg in self.cp_list(cpv_getkey(cpv)):
            if pkg == cpv and (latest is None or latest.build_time < pkg.build_time):
                latest = pkg
    ...
    def cpv_exists(self, mycpv, myrepo=None):
        try:
            return self._instance_key(mycpv, support_string=True) in self.cpvdict
        except KeyError:
    ...
    def cpv_inject(self, mycpv, metadata=None):
        ...
        cp_list = [x for x in cp_list if self._instance_key(x) != instance_key]
    ...
    def aux_get(self, mycpv, wants, myrepo=None):
        metadata = self.cpvdict.get(self._instance_key(mycpv, support_string=True))
        if metadata is None:
    ...
    def aux_update(self, cpv, values):
        self._clear_cache()
        metadata = self.cpvdict.get(self._instance_key(cpv, support_string=True))
        if metadata is None:
            raise KeyError(cpv)
