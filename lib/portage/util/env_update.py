...
from portage.util import (atomic_ofstream, ensure_dirs, getconfig, normalize_path, writemsg, )
...
def env_update(makelinks=1,
    ...
    if vardbapi is None:
        if isinstance(env, config):
            ...
            if hasattr(portage, "db") and target_eroot in portage.db:
                ...
                settings = config(config_root=target_root, target_root=target_root, eprefix=eprefix)
    ...
    vardbapi._fs_lock()
    try:
        return _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level)
    finally:
        vardbapi._fs_unlock()
...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    colon_separated = {
        "ADA_INCLUDE_PATH", "ADA_OBJECTS_PATH", "CLASSPATH", "INFODIR", "INFOPATH", "KDEDIRS", "LDPATH", "MANPATH",
        "PATH", "PKG_CONFIG_PATH", "PRELINK_PATH", "PRELINK_PATH_MASK", "PYTHONPATH", "ROOTPATH",
    }
    ...
    try:
        myld = open(_unicode_encode(ldsoconf_path, encoding=_encodings["fs"], errors="strict"),
    ...
    for lib_dir_glob in ("usr/lib*", "lib*"):
        ...
        for y in glob.glob(_unicode_encode(x, encoding=_encodings["fs"], errors="strict")):
            try:
    ...
    if prelink_capable:
        ...
        try:
            with open(_unicode_encode(prelink_conf, encoding=_encodings["fs"], errors="strict"), "rb", ) as f:
    ...
    if ("CHOST" in settings and "CBUILD" in settings and settings["CHOST"] != settings["CBUILD"]):
    ...
    if makelinks and ldconfig:
        ...
        if ostype == "Linux" or ostype.lower().endswith("gnu"):
            ...
            ret = subprocess.run([ldconfig, "-X", "-r", target_root], cwd="/").returncode
        elif ostype in ("FreeBSD", "DragonFly"):
            writemsg_level(_(f">>> Regenerating {target_root}var/run/ld-elf.so.hints...\n"))
            ret = subprocess.run([
                ldconfig, "-elf", "-i", "-f", f"{target_root}var/run/ld-elf.so.hints", f"{target_root}etc/ld.so.conf",
            ],
                                 cwd="/",
                                 ).returncode
    ...
    systemd_gentoo_env_path = os.path.join(systemd_environment_dir, "10-gentoo-env.conf")
