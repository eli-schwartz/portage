...
def _env_update(makelinks, target_root, prev_mtimes, contents, env, writemsg_level):
    ...
    colon_separated = {
        "ADA_INCLUDE_PATH", "ADA_OBJECTS_PATH", "CLASSPATH", "INFODIR", "INFOPATH", "KDEDIRS",
        "LDPATH", "MANPATH", "PATH", "PKG_CONFIG_PATH", "PRELINK_PATH", "PRELINK_PATH_MASK",
        "PYTHONPATH", "ROOTPATH",
    }
    ...
    if prelink_capable:
        ...
        try:
            with open(_unicode_encode(prelink_conf, encoding=_encodings["fs"], errors="strict"),
                      "rb",
                      ) as f:
                if f.readline(
    ...
    if makelinks and ldconfig:
        ...
        if ostype == "Linux" or ostype.lower().endswith("gnu"):
            ...
            ret = subprocess.run([
                ldconfig, "-elf", "-i", "-f", f"{target_root}var/run/ld-elf.so.hints",
