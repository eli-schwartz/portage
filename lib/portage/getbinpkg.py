...
def make_metadata_dict(data):
    ...
    mydict = {
        k:
        _unicode_decode(portage.xpak.getitem(data, k_bytes),
...
class ParseLinks(html_parser_HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag == "a":
            myarchors = (urllib_parse_unquote(x[1]) for x in attrs
    ...
    if not conn:
        if protocol == "https":
            ...
            if password:
                ...
                sys.stderr.write(
                    colorize("WARN", _(f" * No password provided for username '{username}'\n\n"),
                             ))
                conn.login(username)
...
def dir_get_metadata(baseurl, conn=None, chunk_size=3000, verbose=1, usingcache=1, makepickle=None):
    ...
    try:
        metadatafile = open(
            _unicode_encode(metadatafilename, encoding=_encodings["fs"], errors="strict"), "rb",
        )
    ...
    class CacheStats:
        ...
        def display(self):
            self.out.write("".join(("\r", colorize("WARN",
                                                   _(f"cache miss: '{self.misses}'")), " --- ",
    ...
    for x in tbz2list:
        ...
        if x not in metadata[baseurl]["data"]:
            ...
            for _x in range(3):
                try:
                    myid = file_get_metadata("/".join((baseurl.rstrip("/"), x.lstrip("/"))), conn,
                                             chunk_size)
                    break
    ...
    try:
        if "modified" in metadata[baseurl] and metadata[baseurl]["modified"]:
            metadata[baseurl]["timestamp"] = int(time.time())
            metadatafile = open(
                _unicode_encode(metadatafilename, encoding=_encodings["fs"], errors="strict"), "wb",
            )
            pickle.dump(metadata, metadatafile, protocol=2)
            metadatafile.close()
        if makepickle:
            metadatafile = open(
                _unicode_encode(makepickle, encoding=_encodings["fs"], errors="strict"), "wb",
            )
