...
class PortdbCacheTestCase(TestCase):
    ...
    def testPortdbCache(self):
        ...
        eclasses = {"foo": ("inherit bar", ), "bar": ("IDEPEND=dev-libs/A", ), "baz": ("IDEPEND=", ), }
        ...
        egencache_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "egencache"), "--update-manifests",
                         "--sign-manifests=n", "--repo", "test_repo", "--repositories-configuration",
                         settings.repositories.config_string(),
                         )
        python_cmd = (portage_python, "-b", "-Wd", "-c")

        test_commands = ((lambda: not os.path.exists(pms_cache_dir), ), (lambda: not os.path.exists(md5_cache_dir), ),
				...
				if portage.portdb.repositories['test_repo'].location in portage.portdb._pregen_auxdb:
					sys.exit(1)
			"""), ), egencache_cmd + ("--update", ), (lambda: not os.path.exists(pms_cache_dir), ),
                         (lambda: os.path.exists(md5_cache_dir), ), python_cmd + (textwrap.dedent("""
				import os, sys, portage
			...
			"""), ), python_cmd + (textwrap.dedent("""
				...
				if not isinstance(portage.portdb._pregen_auxdb[portage.portdb.repositories['test_repo'].location], md5_database):
					sys.exit(1)
			"""), ), (BASH_BINARY, "-c",
             "echo %s > %s" % tuple(map(portage._shell_quote, ("cache-formats = md5-dict pms", layout_conf_path,
                                                               ),
                                        )),
             ), egencache_cmd + ("--update", ), (lambda: os.path.exists(md5_cache_dir), ),
                         python_cmd + (textwrap.dedent("""
				import os, sys, portage
				if portage.portdb.repositories['test_repo'].location not in portage.portdb._pregen_auxdb:
					sys.exit(1)
			...
			"""), ),
                         ...
                         (BASH_BINARY, "-c", "echo %s > %s" %
                          tuple(map(portage._shell_quote, ("cache-formats = pms md5-dict", layout_conf_path,
                                                           ),
                                    )),
        ...
        try:
            ...
            for i, args in enumerate(test_commands):
                ...
                self.assertEqual(os.EX_OK, proc.returncode, "command %d failed with args %s" % (i, args, ), )
        finally:
            playground.cleanup()
