...
class PortdbCacheTestCase(TestCase):
    ...
    def testPortdbCache(self):
        ...
        egencache_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "egencache"),
                         "--update-manifests", "--sign-manifests=n", "--repo", "test_repo",
                         "--repositories-configuration", settings.repositories.config_string(),
                         )
        python_cmd = (portage_python, "-b", "-Wd", "-c")

        test_commands = ((lambda: not os.path.exists(pms_cache_dir), ),
			...
			"""), ), (BASH_BINARY, "-c", "echo %s > %s" %
             tuple(map(portage._shell_quote, ("cache-formats = md5-dict pms", layout_conf_path,
                                              ),
                       )),
