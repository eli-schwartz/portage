...
class ShellQuoteTestCase(TestCase):
    ...
    def testShellQuote(self):
        test_data = [
            ...
            ("abc xyz", '"abc xyz"'), ("abc  xyz", '"abc  xyz"'), (" abcxyz ", '" abcxyz "'),
            ("abc\txyz", '"abc\txyz"'), ("abc\t\txyz", '"abc\t\txyz"'), ("\tabcxyz\t", '"\tabcxyz\t"'),
            ("abc\nxyz", '"abc\nxyz"'), ("abc\n\nxyz", '"abc\n\nxyz"'), ("\nabcxyz\n", '"\nabcxyz\n"'),
            # String contains > or <, should be double-quoted to prevent redirection.
            ("abc>xyz", '"abc>xyz"'), ("abc>>xyz", '"abc>>xyz"'), (">abcxyz>", '">abcxyz>"'), ("abc<xyz", '"abc<xyz"'),
            ("abc<<xyz", '"abc<<xyz"'), ("<abcxyz<", '"<abcxyz<"'),
            ...
            # Also double-quoted, though not strictly needed.
            ("abc`xyz", '"abc\\`xyz"'), ("abc``xyz", '"abc\\`\\`xyz"'), ("`abc`", '"\\`abc\\`"'),
