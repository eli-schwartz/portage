...
from portage.package.ebuild.fetch import (ContentHashLayout, DistfileName, _download_suffix, fetch,
                                          FilenameHashLayout, FlatLayout, MirrorLayoutConfig,
                                          )
...
class EbuildFetchTestCase(TestCase):
    ...
    def testEbuildFetch(self):
        ...
        with AsyncHTTPServer(host, content, loop) as server:
            ...
            for cpv, metadata in ebuilds.items():
                ...
                metadata["SRC_URI"] = metadata["SRC_URI"].format(scheme=scheme,
                                                                 host=host,
            ...
            try:
                self._testEbuildFetch(loop, scheme, host, distfiles, ebuilds, content, server,
                                      playground, ro_distdir,
                                      )
            finally:
                shutil.rmtree(ro_distdir)
                playground.cleanup()

    def _testEbuildFetch(self, loop, scheme, host, orig_distfiles, ebuilds, content, server,
                         playground, ro_distdir,
                         ):
        ...
        for layout_lines in mirror_layouts:
            ...
            emirrordist_cmd = (portage._python_interpreter, "-b", "-Wd",
                               os.path.join(str(self.bindir), "emirrordist"), "--distfiles",
                               settings["DISTDIR"], "--config-root", settings["EPREFIX"],
                               "--delete", "--repositories-configuration",
                               settings.repositories.config_string(), "--repo", "test_repo",
                               "--mirror",
                               )
            ...
            proc = loop.run_until_complete(asyncio.create_subprocess_exec(*emirrordist_cmd,
                                                                          env=env))
            ...
            if content_db is not None:
                loop.run_until_complete(
                    self._test_content_db(emirrordist_cmd, env, layouts, content_db, distfiles,
                                          settings, portdb,
                                          ))
    ...
    async def _test_content_db(self, emirrordist_cmd, env, layouts, content_db, distfiles, settings,
                               portdb):
    ...
    def test_filename_hash_layout(self):
        ...
        self.assertEqual(
            FilenameHashLayout("SHA1", "8:16").get_path("foo-1.tar.gz"), "19/c3b6/foo-1.tar.gz",
        )
        self.assertEqual(
...
"""
        mlc.read_from_file(io.StringIO(conf))
        self.assertEqual(mlc.serialize(), (("filename-hash", "INVALID-HASH", "8:16"),
                                           ("filename-hash", "SHA1", "32"), ("flat", ),
                                           ),
                         )
        self.assertIsInstance(mlc.get_best_supported_layout(), FilenameHashLayout)
        self.assertEqual(mlc.get_best_supported_layout().get_path("foo-1.tar.gz"),
...
"""
    ...
    def test_filename_hash_layout_get_filenames(self):
        ...
        layouts = (FlatLayout(), FilenameHashLayout("SHA1", "4"), FilenameHashLayout("SHA1", "8"),
                   FilenameHashLayout("SHA1", "8:16"), FilenameHashLayout("SHA1", "8:16:24"),
