...
"""
        ...
        for binpkg_format in SUPPORTED_GENTOO_BINPKG_FORMATS:
            with self.subTest(binpkg_format=binpkg_format):
                ...
                loop.run_until_complete(
                    asyncio.ensure_future(self._async_test_simple(playground, metadata_xml_files, loop=loop),
                                          loop=loop,
                                          ))
    ...
    async def _async_test_simple(self, playground, metadata_xml_files, loop):
        ...
        egencache_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "egencache"), "--repo",
                         "test_repo", "--repositories-configuration", settings.repositories.config_string(),
                         )
        emerge_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "emerge"), )
        emaint_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.sbindir), "emaint"), )
        env_update_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.sbindir), "env-update"), )
        etc_update_cmd = (BASH_BINARY, os.path.join(str(self.sbindir), "etc-update"))
        fixpackages_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.sbindir), "fixpackages"), )
        portageq_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "portageq"), )
        quickpkg_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "quickpkg"), )
        regenworld_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.sbindir), "regenworld"), )
        ...
        binhost_server = AsyncHTTPServer(binhost_address, BinhostContentMap(binhost_remote_path, binhost_dir),
                                         loop).__enter__()
        binhost_uri = "http://{address}:{port}{path}".format(address=binhost_address,
        ...
        test_commands += (
            emerge_cmd +
            ("--usepkgonly", "--root", cross_root, "--quickpkg-direct=y", "--quickpkg-direct-root", "/", "dev-libs/A",
             ), emerge_cmd + ("--usepkgonly", "--quickpkg-direct=y", "--quickpkg-direct-root", cross_root, "dev-libs/A",
                              ), env_update_cmd, portageq_cmd +
            ("envvar", "-v", "CONFIG_PROTECT", "EROOT", "PORTAGE_CONFIGROOT", "PORTAGE_TMPDIR", "USERLAND",
             ), etc_update_cmd, dispatch_conf_cmd, emerge_cmd + ("--version", ), emerge_cmd + ("--info", ),
            emerge_cmd + ("--info", "--verbose"), emerge_cmd + ("--list-sets", ), emerge_cmd + ("--check-news", ),
            rm_cmd + ("-rf", cachedir), rm_cmd + ("-rf", cachedir_pregen), emerge_cmd + ("--regen", ),
            rm_cmd + ("-rf", cachedir), ({
                "FEATURES": "metadata-transfer"
            }, ) + emerge_cmd + ("--regen", ), rm_cmd + ("-rf", cachedir), ({
                "FEATURES": "metadata-transfer"
            }, ) + emerge_cmd + ("--regen", ), rm_cmd + ("-rf", cachedir),
            egencache_cmd + ("--update", ) + tuple(egencache_extra_args), ({
                "FEATURES": "metadata-transfer"
            }, ) + emerge_cmd + ("--metadata", ), rm_cmd + ("-rf", cachedir), ({
                "FEATURES": "metadata-transfer"
            }, ) + emerge_cmd + ("--metadata", ), emerge_cmd + ("--metadata", ), rm_cmd + ("-rf", cachedir),
            ...
            lambda: self.assertTrue(os.path.exists(os.path.join(pkgdir, "virtual", "foo", foo_filename))), emerge_cmd +
            ("--pretend", "dev-libs/A"), ebuild_cmd + (test_ebuild, "manifest", "clean", "package", "merge"),
            emerge_cmd + ("--pretend", "--tree", "--complete-graph", "dev-libs/A"), emerge_cmd + ("-p", "dev-libs/B"),
            emerge_cmd + ("-p", "--newrepo", "dev-libs/B"), emerge_cmd + ("-B", "dev-libs/B",
                                                                          ),
            emerge_cmd + ("--oneshot", "--usepkg", "dev-libs/B",
                          ),
            # trigger clean prior to pkg_pretend as in bug #390711
            ebuild_cmd + (test_ebuild, "unpack"), emerge_cmd + ("--oneshot", "dev-libs/A",
            ...
            emerge_cmd + ("--pretend", "--depclean",
                          ), emerge_cmd + ("--depclean", ), quickpkg_cmd + ("--include-config", "y", "dev-libs/A",
                                                                            ),
            # Test bug #523684, where a file renamed or removed by the
            # admin forces replacement files to be merged with config
            # protection.
            lambda: self.assertEqual(
                0, len(list(find_updated_config_files(eroot, shlex_split(settings["CONFIG_PROTECT"])))),
            ), lambda: os.unlink(os.path.join(eprefix, "etc", "A-0")), emerge_cmd + ("--usepkgonly", "dev-libs/A"),
            lambda: self.assertEqual(
                1, len(list(find_updated_config_files(eroot, shlex_split(settings["CONFIG_PROTECT"])))),
            ), emaint_cmd + ("--check", "all"), emaint_cmd + ("--fix", "all"), fixpackages_cmd, regenworld_cmd,
            portageq_cmd + ("match", eroot, "dev-libs/A"), portageq_cmd + ("best_visible", eroot, "dev-libs/A"),
            ...
            portageq_cmd + ("metadata", eroot, "installed", "dev-libs/A-1", "EAPI", "USE", "RDEPEND",
                            ), portageq_cmd + ("owners", eroot, eroot + "usr"), emerge_cmd + ("-p", eroot + "usr"),
            emerge_cmd + ("-p", "--unmerge", "-q", eroot + "usr"), emerge_cmd + ("--unmerge", "--quiet", "dev-libs/A"),
            ...
            ({
                ...
                "ROOT": cross_root
            }, ) + emerge_cmd + ("dev-libs/D", ), portageq_cmd + ("has_version", cross_eroot, "dev-libs/D"),
            # EAPI 5
            ({
                "EPREFIX": cross_prefix
            ...
            ({
                "ROOT": cross_root
            }, ) + emerge_cmd + ("dev-libs/B", ), portageq_cmd + ("has_version", cross_eroot, "dev-libs/B"),
        )
        ...
        if fetch_bin is not None:
            test_commands = test_commands + (
                lambda: os.rename(pkgdir, binhost_dir), emerge_cmd + ("-e", "--getbinpkgonly", "dev-libs/A"),
                lambda: shutil.rmtree(pkgdir), lambda: os.rename(binhost_dir, pkgdir),
                # Remove binrepos.conf and test PORTAGE_BINHOST.
                lambda: os.unlink(binrepos_conf_file), lambda: os.rename(pkgdir, binhost_dir),
                ({
                    "PORTAGE_BINHOST": binhost_uri
                }, ) + emerge_cmd + ("-fe", "--getbinpkgonly", "dev-libs/A"), lambda: shutil.rmtree(pkgdir),
        ...
        dirs = [
            cachedir, cachedir_pregen, cross_eroot, cross_prefix, distdir, fake_bin, portage_tmpdir, updates_dir,
