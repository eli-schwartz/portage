...
"""
        ...
        for binpkg_format in SUPPORTED_GENTOO_BINPKG_FORMATS:
            with self.subTest(binpkg_format=binpkg_format):
                ...
                loop.run_until_complete(
                    asyncio.ensure_future(self._async_test_simple(playground,
                                                                  metadata_xml_files,
                                                                  loop=loop),
                                          loop=loop,
                                          ))
    ...
    async def _async_test_simple(self, playground, metadata_xml_files, loop):
        ...
        egencache_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.bindir), "egencache"),
                         "--repo", "test_repo", "--repositories-configuration",
        ...
        env_update_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.sbindir),
                                                                    "env-update"),
                          )
        ...
        regenworld_cmd = (portage_python, "-b", "-Wd", os.path.join(str(self.sbindir),
                                                                    "regenworld"),
                          )
        ...
        test_commands += (
            emerge_cmd + ("--usepkgonly", "--root", cross_root, "--quickpkg-direct=y",
                          "--quickpkg-direct-root", "/", "dev-libs/A",
                          ), emerge_cmd + ("--usepkgonly", "--quickpkg-direct=y",
                                           "--quickpkg-direct-root", cross_root, "dev-libs/A",
                                           ), env_update_cmd,
            portageq_cmd + ("envvar", "-v", "CONFIG_PROTECT", "EROOT", "PORTAGE_CONFIGROOT",
                            "PORTAGE_TMPDIR", "USERLAND",
                            ), etc_update_cmd, dispatch_conf_cmd, emerge_cmd + ("--version", ),
            emerge_cmd + ("--info", ), emerge_cmd + ("--info", "--verbose"),
            emerge_cmd + ("--list-sets", ), emerge_cmd + ("--check-news", ),
            rm_cmd + ("-rf", cachedir), rm_cmd + ("-rf", cachedir_pregen),
            ...
            egencache_cmd + ("--update", ) + tuple(egencache_extra_args), ({
                ...
                "FEATURES":
                "metadata-transfer"
            }, ) + emerge_cmd + ("--metadata", ), emerge_cmd + ("--metadata", ), rm_cmd +
            ("-rf", cachedir), emerge_cmd + ("--oneshot", "virtual/foo"), lambda: self.assertFalse(
                os.path.exists(os.path.join(pkgdir, "virtual", "foo", foo_filename))), ({
                    "FEATURES":
                    "unmerge-backup"
                }, ) + emerge_cmd + ("--unmerge", "virtual/foo"), lambda: self.assertTrue(
                    os.path.exists(os.path.join(pkgdir, "virtual", "foo", foo_filename))),
            emerge_cmd + ("--pretend", "dev-libs/A"),
            ebuild_cmd + (test_ebuild, "manifest", "clean", "package", "merge"),
            emerge_cmd + ("--pretend", "--tree", "--complete-graph", "dev-libs/A"),
            emerge_cmd + ("-p", "dev-libs/B"), emerge_cmd + ("-p", "--newrepo", "dev-libs/B"),
            ...
            emerge_cmd + ("--usepkgonly", "dev-libs/A"), lambda: self.assertEqual(
                1,
                len(list(find_updated_config_files(eroot, shlex_split(settings["CONFIG_PROTECT"])))
                    ),
            ), emaint_cmd + ("--check", "all"), emaint_cmd + ("--fix", "all"), fixpackages_cmd,
            regenworld_cmd, portageq_cmd + ("match", eroot, "dev-libs/A"),
            portageq_cmd + ("best_visible", eroot, "dev-libs/A"),
            portageq_cmd + ("best_visible", eroot, "binary", "dev-libs/A"),
            portageq_cmd + ("contents", eroot, "dev-libs/A-1"), portageq_cmd +
            ("metadata", eroot, "ebuild", "dev-libs/A-1", "EAPI", "IUSE", "RDEPEND"), portageq_cmd +
            ("metadata", eroot, "binary", "dev-libs/A-1", "EAPI", "USE", "RDEPEND"), portageq_cmd +
            ("metadata", eroot, "installed", "dev-libs/A-1", "EAPI", "USE", "RDEPEND",
             ), portageq_cmd + ("owners", eroot, eroot + "usr"), emerge_cmd + ("-p", eroot + "usr"),
            emerge_cmd + ("-p", "--unmerge", "-q", eroot + "usr"), emerge_cmd +
            ("--unmerge", "--quiet", "dev-libs/A"), emerge_cmd + ("-C", "--quiet", "dev-libs/B"),
        ...
        if fetch_bin is not None:
            test_commands = test_commands + (
                lambda: os.rename(pkgdir, binhost_dir), emerge_cmd +
                ("-e", "--getbinpkgonly", "dev-libs/A"), lambda: shutil.rmtree(pkgdir),
        ...
        dirs = [
            cachedir, cachedir_pregen, cross_eroot, cross_prefix, distdir, fake_bin, portage_tmpdir,
