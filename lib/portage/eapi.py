...
_eapi_attrs = collections.namedtuple(
    "_eapi_attrs",
    ("allows_package_provided", "bdepend", "broot", "exports_AA", "exports_EBUILD_PHASE_FUNC", "exports_ECLASSDIR",
     "exports_KV", "exports_merge_type", "exports_PORTDIR", "exports_replace_vars", "feature_flag_test", "idepend",
     "iuse_defaults", "iuse_effective", "posixish_locale", "path_variables_end_with_trailing_slash", "prefix",
     "repo_deps", "required_use", "required_use_at_most_one_of", "selective_src_uri_restriction", "slot_operator",
     "slot_deps", "src_uri_arrows", "strong_blocks", "use_deps", "use_dep_defaults", "empty_groups_always_true",
     "sysroot",
     ),
)
...
def _get_eapi_attrs(eapi_str: Optional[str]) -> _eapi_attrs:
    ...
    if eapi_str is None or not eapi_is_supported(eapi_str):
        return _eapi_attrs(allows_package_provided=True,
                           bdepend=False,
                           broot=True,
        ...
        eapi = Eapi(eapi_str)
        return _eapi_attrs(allows_package_provided=eapi <= Eapi("6"),
                           bdepend=eapi >= Eapi("7"),
                           broot=eapi >= Eapi("7"),
