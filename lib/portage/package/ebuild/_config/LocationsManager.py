...
from portage.util import (ensure_dirs, grabfile, normalize_path, read_corresponding_eapi_file,
                          shlex_split, writemsg,
                          )
from portage.util._path import exists_raise_eaccess, isdir_raise_eaccess
from portage.repository.config import (parse_layout_conf, _portage1_profiles_allow_directories,
                                       _profile_node,
                                       )

_PORTAGE1_DIRECTORIES = frozenset([
    "package.mask", "package.provided", "package.use", "package.use.mask", "package.use.force",
    "use.mask", "use.force",
])
...
class LocationsManager:
    ...
    def __init__(self,
        ...
        self.config_root = normalize_path(os.path.abspath(self.config_root or os.sep)).rstrip(
            os.sep) + os.sep
        ...
        if self.sysroot is None:
            ...
            self.sysroot = normalize_path(os.path.abspath(self.sysroot or os.sep)).rstrip(
                os.sep) + os.sep
    ...
    def load_profiles(self, repositories, known_repository_paths):
        ...
        if self.config_profile_path is None:
            deprecated_profile_path = os.path.join(self.config_root, "etc", "make.profile")
            self.config_profile_path = os.path.join(self.config_root, PROFILE_PATH)
            if isdir_raise_eaccess(self.config_profile_path):
                self.profile_path = self.config_profile_path
                if isdir_raise_eaccess(deprecated_profile_path) and not os.path.samefile(
    ...
    def _addProfile(self, currentPath, repositories, known_repos, previous_repos):
        ...
        intersecting_repos = tuple(x for x in known_repos if current_abs_path.startswith(x[0]))
        ...
        if intersecting_repos:
            allow_directories = eapi_allows_directories_on_profile_level_and_repository_level(
        ...
        if compat_mode:
            ...
            offenders = sorted(x for x in offenders if os.path.isdir(os.path.join(currentPath, x)))
            if offenders:
                warnings.warn(
                    _("\nThe selected profile is implicitly using the 'portage-1' format:\n"
                      ...
                      "\tprofile-formats = portage-1") % dict(profile_path=currentPath,
                                                              repo_name=repo_loc,
                                                              files="\n\t".join(offenders),
                                                              ))
        ...
        if exists_raise_eaccess(parentsFile):
            ...
            for parentPath in parents:
                ...
                if not abs_parent and allow_parent_colon:
                    parentPath = self._expand_parent_colon(parentsFile, parentPath, repo_loc,
                                                           repositories)
    ...
    def set_root_override(self, root_overwrite=None):
        ...
        self.target_root = normalize_path(os.path.abspath(self.target_root)).rstrip(
