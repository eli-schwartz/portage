...
class SVNSync(NewBase):
    ...
    def new(self, **kwargs):
        ...
        exitcode = portage.process.spawn_bash(
            "cd %s; exec svn co %s ." %
            (portage._shell_quote(self.repo.location), portage._shell_quote(svn_root),
             ), **self.spawn_kwargs,
        )
    ...
    def update(self):
        ...
        # svn update
        exitcode = portage.process.spawn_bash(
            f"cd {portage._shell_quote(self.repo.location)}; exec svn update", **self.spawn_kwargs,
        )
        if exitcode != os.EX_OK:
    ...
    def _svn_upgrade(self):
        ...
        """
        exitcode = portage.process.spawn_bash(
            f"cd {portage._shell_quote(self.repo.location)}; exec svn upgrade", **self.spawn_kwargs,
        )
        if exitcode != os.EX_OK:
